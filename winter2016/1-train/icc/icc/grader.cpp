...
void Interaction::SetScore(int score, const string& message) {
    ...
    if (PRINT_SCORE_TO_STDERR) { fprintf(stderr, "%d %s\n", score, message.c_str()); }
...
*/
    ...
    ifstream fin("icc.in");
    int solver_type; fin >> solver_type;
    int max_steps, max_points; fin >> max_steps >> max_points;
    Solver* solver;

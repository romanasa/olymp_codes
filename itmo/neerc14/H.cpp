...
struct myV {
	...
	void assign(int len, int x) {
		if (a == 0) a = new int[len];
		memset(a, sizeof(int) * len, 0);
	}
	...
	myV& operator= (const myV &other) {
		mn = other.mn;
		mx = other.mx;
		a = new int[2 * d + 7];
		memcpy(a, other.a, sizeof(int) * (2 * d + 7));
		return *this;
	}
...
void upd(int st, int nc) {
	...
	for (int i = (int)pr.size() - 2; i >= 0; i--) {
		...
		for (int i = M[v].mn; i <= M[v].mx; i++) {
			...
			M[p[v]].mn = min(M[p[v]].mn, i + cost[v]);
			M[p[v]].mx = max(M[p[v]].mx, i + cost[v]);
		}
